  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 rounded shadow-md border border-gray-200">
        <p className="font-bold">{date.toLocaleDateString('default', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
...
const MoodChart = ({ data }: { data: any[] }) => (
  <ResponsiveContainer width="100%" height={200}>
    <LineChart data={data}>
      <XAxis dataKey="date" tickFormatter={formatXAxis} />
      <YAxis dataKey="mood" type="category" ticks={moodOrder} />
      <Tooltip content={<CustomTooltip />} />
      <Line type="monotone" dataKey="mood" strokeWidth={2} dot={{ r: 6 }} name="Mood" unit="">
        {data.map((entry, index) => (
          <Line key={`dot-${index}`} dataKey="mood" stroke={moodColors[entry.mood as keyof typeof moodColors]} />
        ))}
      </Line>
    </LineChart>
  </ResponsiveContainer>
...
export function VitalsTracking({ patientId }: VitalsTrackingProps) {
      ...
      <div className="container mx-auto bg-white rounded-lg shadow-lg overflow-hidden max-w-6xl">
        <div className="bg-blue-600 text-white p-6">
          <div className="flex justify-between items-center mb-4">
            <Link href={`/patients/${patientId}`} className="inline-flex items-center text-white hover:underline transition-colors duration-200">
